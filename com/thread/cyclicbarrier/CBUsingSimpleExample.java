 ...
 * Alternative of :
 *
 * When to use : It can be used to perform final part of task once individual tasks are completed. The barrier can be re used by calling reset. 
 *
 * Example description : Suppose we want to download a big file from internet then we can divide the entire into some part
 *                       and distribute those part among different thread to start download separately and only when all the 
 *                       download is finished then we can start merging all the parts to get the original file.                    
 *                       
 *
...
public class CBUsingSimpleExample {
	...
	private class Worker implements Runnable {
		...
		public void run() {
			...
			// So if we want to multiple operation we have to call multiple await after doing that operation and we get that many call to CB run method. 
			waitOnBarrier();
		}
		...
		private void waitOnBarrier(){
			try {
				...
				System.out.println("**********Subrata :: "+Thread.currentThread().getName()+" InterruptedException !!");
			} catch (BrokenBarrierException e) {
				System.out.println("**********Subrata :: "+Thread.currentThread().getName()+" BrokenBarrierException !!");
			}
		}
		
		private void doSomeOtherTask(){
			System.out
					.println("**********Subrata :: CBUsingSimpleExample.Worker.doSomeOtherTask() by ::"+cb.getNumberWaiting());
			waitOnBarrier();
